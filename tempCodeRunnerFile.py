"""
VisionDrive - Self-Calibrating Gaze Tracking
Automatically calibrates to YOUR eye characteristics with manual adjustment options
"""
...
class Config:
    ...
    GAZE_CENTER_MIN = 0.60  # Start wider based on your 0.718 reading
    GAZE_CENTER_MAX = 0.80  # Adjusted for your eye position
...
class CalibratingGazeTracker:
    """
    Self-calibrating gaze tracker with manual adjustment
    ...
    def __init__(self):
        ...
        self.calibration_target = 30  # Number of samples for calibration
        
        # Adjustable thresholds
        self.center_min = Config.GAZE_CENTER_MIN
        self.center_max = Config.GAZE_CENTER_MAX
        self.threshold_adjustment = 0.02  # How much to adjust per key press
    ...
    def refresh(self, frame):
        ...
        if current_ratio is not None:
            ...
            if self.calibrating and len(self.calibration_samples) < self.calibration_target:
    ...
    def complete_calibration(self):
        """Complete calibration and set thresholds based on collected data"""
        if len(self.calibration_samples) < 10:
            print("⚠ Not enough calibration samples, using defaults")
            self.calibrating = False
            return
        
        # Calculate center position from calibration samples
        center_position = sum(self.calibration_samples) / len(self.calibration_samples)
        
        # Set thresholds with a margin around the center position
        ...
        print(f"  Center position: {center_position:.3f}")
        print(f"  Center zone: {self.center_min:.3f} to {self.center_max:.3f}")
        print(f"  Left: < {self.center_min:.3f}")
        print(f"  Right: > {self.center_max:.3f}")
        print("="*50 + "\n")
    ...
    def get_state(self):
        ...
        # Use calibrated thresholds (REVERSED for correct direction)
        if ratio < self.center_min:
            return "LEFT"  # Low ratio = LEFT
        elif ratio > self.center_max:
            return "RIGHT"  # High ratio = RIGHT
        else:
            return "CENTER"
    ...
    def get_debug_info(self):
        ...
        return {
            ...
            'calibration_progress': len(self.calibration_samples) if self.calibrating else self.calibration_target
        }
...
class VisionDriveApp:
    ...
    def initialize(self):
        print("\n" + "="*50)
        print("VisionDrive - SELF-CALIBRATING Gaze Control")
        print("="*50)
        ...
        print("\n[2/4] Initializing CALIBRATING Gaze Tracker...")
        self.gaze_tracker = CalibratingGazeTracker()
        print("✓ Gaze tracker ready - will auto-calibrate")
        ...
        print("  2. Keep your head still for 3 seconds")
        print("  3. System will auto-calibrate to YOUR eyes")
        print("\nManual Adjustment Keys:")
        print("  • Press 'C' to recalibrate")
        print("  • Press '←' to adjust LEFT threshold down")
        print("  • Press '→' to adjust LEFT threshold up")
        print("  • Press '[' to adjust RIGHT threshold down")
        print("  • Press ']' to adjust RIGHT threshold up")
    ...
    def draw_calibration_overlay(self, frame):
        ...
        progress = debug_info['calibration_progress']
        target = self.gaze_tracker.calibration_target
        ...
        # Calibration text
        y = h//3 + 50
        cv2.putText(frame, "CALIBRATING GAZE", (w//4 + 20, y),
                    cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 255), 2)
        y += 50
        cv2.putText(frame, "Look straight at camera", (w//4 + 20, y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
        y += 40
        cv2.putText(frame, "Keep your head still", (w//4 + 20, y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
        ...
        bar_width = w//2 - 40
        bar_x = w//4 + 20
        progress_width = int(bar_width * (progress / target))
        
        cv2.rectangle(frame, (bar_x, y), (bar_x + bar_width, y + 20), (100, 100, 100), -1)
        cv2.rectangle(frame, (bar_x, y), (bar_x + progress_width, y + 20), (0, 255, 0), -1)
        
        cv2.putText(frame, f"{progress}/{target}", (bar_x + bar_width//2 - 30, y + 15),
    ...
    def draw_ui(self, frame, gaze_state, hand_gear, command, fps, finger_states):
        ...
        if gaze_state == "LEFT":
            gaze_color = (200, 100, 0)  # Blue for LEFT
        elif gaze_state == "RIGHT":
            gaze_color = (0, 100, 200)  # Red for RIGHT
        elif gaze_state == "CENTER":
            gaze_color = (0, 255, 0)  # Green for CENTER
        ...
        if self.show_debug:
            debug_info = self.gaze_tracker.get_debug_info()
            debug_y = h - 120
            cv2.putText(frame, "DEBUG INFO:", (w - 250, debug_y),
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.gaze_tracker.calibrating:
                debug_info = self.gaze_tracker.get_debug_info()
                if debug_info['calibration_progress'] >= self.gaze_tracker.calibration_target:
                    self.gaze_tracker.complete_calibration()
            ...
            # Show calibration overlay if calibrating
            if self.gaze_tracker.calibrating:
                frame_display = self.draw_calibration_overlay(frame_display)
            ...
            cv2.imshow("VisionDrive - Self-Calibrating Control", frame_display)
            ...
            if key == 27:  # ESC
                ...
                print("\n🎯 Recalibrating... Look straight ahead!")
                self.gaze_tracker.calibrating = True
                self.gaze_tracker.calibration_samples = []
